...
#include <stdio.h>
// the three operands are encoded inside a single unsigned short. This method cracks them open
static void inline decode_ops(unsigned short input, byte * dest, byte * src1, byte * src2) {
  *dest = input & 0x001F;
  *src1 = (input & 0x03E0) >> 5;
  ...
  // with the value coming out of the this stage. (also, the read is not on zero)
	if (core->mys.right.Rdest != 0 && core->mys.right.control()->register_write) {
		if (core->exs.right.Rdest != core->ids.right.Rsrc1 && 
			...
			core->ids.right.Rsrc1Val =  core->mys.right.control()->mem_read?core->mys.right.mem_data:core->mys.right.aluresult; 
		...
		if (core->exs.right.Rdest != core->ids.right.Rsrc2 && 
			...
			core->ids.right.Rsrc2Val =  core->mys.right.control()->mem_read?core->mys.right.mem_data:core->mys.right.aluresult;  
